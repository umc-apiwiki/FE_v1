...
const ProfilePage = () => {
  ...
  if (isLoading) return (
    <>
      <MobileHeader />
      <div className="pt-24 xs:pt-28 md:pt-32 text-center pb-20 text-sm xs:text-base">로딩 중...</div>
  ...
  if (error) return (
    <>
      <MobileHeader />
      <div className="pt-24 xs:pt-28 md:pt-32 text-center text-red-500 pb-20 text-sm xs:text-base">에러 발생</div>
      <MobileBottomNavigation />
    </>
  )
    ...
    <>
      ...
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] relative overflow-hidden pt-20 xs:pt-16 md:pt-12 pb-24 xs:pb-28 px-4">
        ...
        >
          ...
          >
            <img src={editIcon} alt="Edit Profile" className="w-5 h-5 xs:w-6 xs:h-6 sm:w-7 sm:h-7" />
          </motion.button>
        </motion.div>
